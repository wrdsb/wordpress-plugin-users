 ...
 * Access users
 */
class WRDSB_REST_User_by_Username_Controller extends WP_REST_Controller {
	...
	public function register_routes() {
		...
		register_rest_route( $this->namespace, '/' . $this->rest_base . '/(?P<username>[a-zA-Z0-9-]+)', array(
			array(
	...
	public function get_item_permissions_check( $request ) {
		...
		if ( empty( $username ) || empty( $user->ID ) ) {
	...
	public function create_item( $request ) {
		...
		if ( is_multisite() ) {
			$ret = wpmu_validate_user_signup( $user->user_login, $user->user_login );
		...
		if ( is_multisite() ) {
			$user_id = wpmu_create_user( $user->user_login, $user->user_pass, $user->user_login );
			if ( ! $user_id ) {
